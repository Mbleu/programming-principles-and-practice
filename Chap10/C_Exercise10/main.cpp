// Calculator Program modified to input and output from file
// Last changed 02/07/23 - William Falcon-Uff
...
const char from = 'F';		// symbolic constants, the value of which is fairly meaningless but allows for the use of the constant name
const char to = 'T';
const char let = 'L';
const char perm = 'P';
const char assign = 'A';
...
{
	if (ifs.is_open()) {																// if a file is open and has reached EOF, close it and reset input to console
		is->peek();
		if (!is->good()) {
			ifs.close();
			is = &cin;
	...
	do {																				// discard all whitespace apart from newline
		is->get(ch);
		if (ch < 0) error("Bad token");													// necessary because isspace and similar functions only accepts unsigned chars
	} while (isspace(ch) && ch != '\n');
	switch (ch) {
	case '\n':
	case ';':
	...
	*is >> val;
	...
	default:
		if (isalpha(ch) || ch == '_') {													// variable names must start with a letter or underscore
			string s;
			s += ch;
			while (is->get(ch) && (isalpha(ch) || isdigit(ch) || ch == '_' || ch == '.')) s += ch;	// variable names can contain letters, numbers, underscores and fullstops
...
void redirection()	// sets a new i/ostream for statements
{
	Token t = ts.get();
	if (t.kind != from && t.kind != to) error("keyword expected in redirection"); // pre
	Token t2 = ts.get();
	if (t2.kind != name) error("name expected in redirection");	// check that the token is indeed of type name (of a file)
	string filename = t2.name;
	switch (t.kind) {
	case from:													// read from file until eof
		ts.ifs.open(t2.name);
		if (!ts.ifs) error("file not found");
		ts.is = &ts.ifs;
		break;
	case to:													// write to console or file until changed
		if (t2.name == "console") {
			ofs.close();
			os = &cout;
		}
			...
			ofs.open(t2.name);
			if (!ofs) error("couldn't open file");
			os = &ofs;
		}
		break;
	}
...
void print_help()	// displays instructions for using the calculator
{
	cout << "An expression can contain: numbers(integers and decimals), operators and variables.\n"
		...
		<< "To change the input, use the 'from' keyword and provide a file name.\n"
		<< "To change the output, use the 'to' keyword and provide a file name or \"console\".\n"
		<< "Type 'quit' to exit.\n";
	return;
}
...
{
	while (true) {
		if (!ts.ifs.is_open()) cout << prompt;			// don't print prompt during file reads
		bool seq_end = false;
		while (!seq_end) try {
			Token t = ts.get();
			switch (t.kind) {
			...
			default:
				ts.unget(t);
				*os << result << statement() << endl;	// action the statement
				break;
			}
		}
