...
// Numeral tables
const std::vector<std::string> units{ "I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX" };		// numerals representing 1-9 for each decimal place
const std::vector<std::string> tens{ "X", "XX", "XXX", "XL", "L", "LX", "LXX", "LXXX", "XC" };		// there is no concept of 0
const std::vector<std::string> hundreds{ "C", "CC", "CCC", "CD", "D", "DC", "DCC", "DCCC", "CM" };	// a valid numeral is made by appending up to one numeral from each
const std::vector<std::string> thousands{ "M", "MM", "MMM" };
const std::vector<std::vector<std::string>>& dec_places{ units, tens, hundreds, thousands };		// combines tables for easy searches
