...
Program that uses the integer only version of the calculator program from chapter 7, exercise 10 and the roman numeral program from the previous exercise.
Not optimised, only the tokenizer and the output were minorly altered to exhibit the encapulation of these two programs.*/

// Calculator Program (Roman Numeral Only Version)
// Last changed 01/07/23 - William Falcon-Uff
...
Token Token_stream::get()	// gets and returns the next Token, first checks the buffer otherwise creates one. It handles raw text.
{
	...
	case 'D':
	case 'M':
	{																					// its a number, so create a Token of the whole number
	string numeral{ ch };
	while (cin.get(ch) && (ch == 'I' || ch == 'V' || ch == 'X' || ch == 'L'				// get the whole numeral
		|| ch == 'C' || ch == 'D' || ch == 'M')) numeral += ch;
	cin.unget();
	unsigned int val{ Roman::to_int(numeral) };											// convert the numeral to an int												
...
double expression()	// evaluates an expression: term, expression + term, expression - term
{
	...
	while (true) {
		...
		default:							// no more + or - found, return complete evaluated term
			ts.unget(t);
			return narrow_cast<int>(left);	// enforce integer only solutions
		}
	}
}
...
{
	cout << "An expression can contain: numbers(numerals only), operators and variables.\n"
		<< "An operator can be: + - * / % () pow(a,b) sqrt()\n"
		<< "pow(a,b) will raise a to the power b. sqrt() will give the square root.\n"
		<< "To declare a variable, use the 'let' keyword and provide a name and value. e.g. let x = XVIII\n"
...
void calculate()	// calculator that prompts the user to input and ouputs a solution
{
	while (true) {
		...
		while (!seq_end) try {
			...
			default:
				ts.unget(t);
				cout << result << Roman::to_numeral(narrow_cast<int>(statement())) << endl;	// action the statement, the narrowcast will check for inprecisions
				break;
			}
		}
