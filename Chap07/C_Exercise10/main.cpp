// Calculator Program (Integers Only Version)
...
Token Token_stream::get()	// gets and returns the next Token, first checks the buffer otherwise creates one. It handles raw text.
{
	...
	double val;
	cin >> val;
	int ival = narrow_cast<int>(val);													// enforce integer only input
...
{
	cout << "An expression can contain: numbers(integers only), operators and variables.\n"
		<< "An operator can be: + - * / % () pow(a,b) sqrt()\n"
		<< "pow(a,b) will raise a to the power b. sqrt() will give the square root.\n"
		<< "To declare a variable, use the 'let' keyword and provide a name and value. e.g. let x = 10\n"
